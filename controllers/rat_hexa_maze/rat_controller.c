...
// wheels speed
const double NORMAL_SPEED = 20.0;
const double RUSH_SPEED = 40.0;
...
//gps
WbDeviceTag gps;
double previous_location[3] = {0.0, 0.0, 0.0};
bool stucked = false;
...
int handle_keyboard(int key) {
  ...
  switch (key) {
    ...
    case WB_ROBOT_KEYBOARD_LEFT: // turn left [cursor left]
      wb_differential_wheels_set_speed(-NORMAL_SPEED/4, NORMAL_SPEED/4);
      break;
...
// panic_mode: the rat is stucked; this mode try to unblock it
//             by avoid a strange behavior
int panic_mode() {
  log_info("panic mode");
  int state = (int)biological_clock;
  switch (state % 8) {
    ...
    case 5:
      wb_differential_wheels_set_speed(-NORMAL_SPEED/4, NORMAL_SPEED/4);
      break;
    default: //probability to leave this loop: 1/4
      stucked = false;
      break;
  };
  animate_whiskers();
...
int run_step() {
  ...
  // read and process gps
  double *gps_values = wb_gps_get_values(gps);
  if (memcmp(gps_values,previous_location,3*sizeof(double))) {
    //check if the robot is stucked
    if ( 0.02 > sqrt( pow(gps_values[0]-previous_location[0],2) +
               pow(gps_values[2]-previous_location[2],2)))
      stucked = true;
    memcpy(previous_location,gps_values,3*sizeof(double));
  }
