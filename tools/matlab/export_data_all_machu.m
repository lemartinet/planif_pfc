...
Q = zeros(length(N), 13);
for i = 1:length(N)
    fprintf('%d%s...', floor(100*i/length(N)), '%');
    fr = C{i}; 
    mfr = sum(fr(:) .* probav(:));
    ...
    info = mutual_information(tfr(idx), probav(idx));
    infoEqui = mutual_information(tfr(idx), ones(length(idx),1) / length(idx));
    fr={};
    ...
    %Q(i, 9:11) = [ spatialInfo, sparsity, specificity ];
end
names = {'fsize' 'mfr', 'sfr' 'meanFr' 'stdFr' 'lks' 'info' 'infoEqui' ...
    'spatialInfo' 'sparsity' 'specificity' 'sk' 'skcut'};
save  ~/Dropbox/share/karim/neuron_data/Q.mat Q names;
fprintf('\n');
